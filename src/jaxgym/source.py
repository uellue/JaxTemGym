...
def ParallelBeam(
    ...
    input_matrix = input_matrix.at[:, 2].set(0.0 + tilt_yx[1])
    input_matrix = input_matrix.at[:, 3].set(0.0 + tilt_yx[0])
...
def RadialSpikesBeam(
    z, radius, n_rays, tilt_yx=(0.0, 0.0), centre_yx=(0.0, 0.0), voltage=None
):
    # Build radial spikes
    n_each = max(n_rays // 4, 1)
    xvals = jnp.linspace(0.0, radius, num=n_each, endpoint=True)
...
def XPointBeam(
    ...
    if random:
        ...
        tilts = jax.random.uniform(
            key, (n_rays,), minval=-semi_angle, maxval=semi_angle
        )
    else:
